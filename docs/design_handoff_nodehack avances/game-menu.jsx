...
function MenuScreen({ onPlay, onDeck, onNucleo, nucleo, decks, favDeck }) {
  return (
    <div className="screen menu-screen">
      <MatrixRain intensity={4} />
      ...
      <nav className="menu-nav">
        <button className="menu-btn primary" onClick={onPlay}>
          ...
          <span className="menu-btn-body">
            ...
            <span className="menu-btn-sub">{favDeck ? `Mazo: ${favDeck.name}` : 'Duelo de resolución simultánea'}</span>
          </span>
        ...
        <button className="menu-btn" onClick={onDeck}>
          <span className="menu-btn-glyph">▤</span>
          <span className="menu-btn-body">
            <span className="menu-btn-label">MIS MAZOS</span>
            <span className="menu-btn-sub">{decks && decks.length ? `${decks.length} mazo${decks.length > 1 ? 's' : ''}${favDeck ? ` · ★ ${favDeck.name}` : ''}` : 'Crea tu primer mazo'}</span>
          </span>
          <span className="menu-btn-arrow">▸</span>
        </button>
